...
$sql = "SELECT * FROM posts WHERE usersEmail='{$_SESSION['userEmail']}' ORDER BY postID DESC";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        // Display the delete button, if the post was created by the logged in user
        if ($row['usersEmail'] == $_SESSION['userEmail']) {
        ...
        if (isset($_GET['deletePost']) && $_GET['deletePost'] == 'success') {
          echo '<p class="success-message">Post successfully deleted.</p>';
